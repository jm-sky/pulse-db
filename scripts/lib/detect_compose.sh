...
# Optional: APP_CONTAINER_NAME (default: pulse-db-app).
...
APP_CONTAINER_NAME="${APP_CONTAINER_NAME:-pulse-db-app}"
...
detect_compose_context() {
  ...
  # Fallback priority: root dev stack, then other root files, then legacy backend/.
