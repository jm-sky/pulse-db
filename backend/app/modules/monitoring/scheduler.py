...
- Both engines get the full diagnostic tick set (session sample, query
  stats, query plans, blocking, deadlocks, indexes, rollups) once Phase 1
  elements 1–5 are implemented for each; `trivial` remains the shared
  health-check tick.
...
from .collector import (
    run_blocking_collection,
    run_deadlocks_collection,
    run_indexes_collection,
...
_DIAGNOSTIC_TICKS: tuple[TickSpec, ...] = (
    ...
    TickSpec("blocking", 30.0, functools.partial(run_blocking_collection, interval_ms=30_000)),
    TickSpec("deadlocks", 60.0, functools.partial(run_deadlocks_collection, interval_ms=60_000)),
    TickSpec("indexes", 86400.0, functools.partial(run_indexes_collection, interval_ms=86_400_000)),
