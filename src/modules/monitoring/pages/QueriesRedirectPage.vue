...
watch(
  ...
  () => {
    ...
    if (first) {
      void router.replace(MonitoringRoutePaths.instanceQueries(first.id))
    }
  },
  { immediate: true },
)
...
<template>
  <div class="flex h-dvh items-center justify-center text-sm text-muted-foreground">
    <span v-if="isError">{{ t('monitoring.queries.loadInstancesError') }}</span>
    <span v-else-if="isPending || !isSuccess || (data?.instances?.length ?? 0) > 0">
      {{ t('monitoring.queries.loadingInstances') }}
    </span>
    <span v-else>{{ t('monitoring.queries.noInstances') }}</span>
  </div>
</template>
