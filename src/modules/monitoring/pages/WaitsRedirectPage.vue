...
watch(
  ...
  () => {
    ...
    if (first) {
      void router.replace(MonitoringRoutePaths.instanceWaits(first.id))
    }
  },
  { immediate: true },
)
...
<template>
  <div class="flex h-dvh items-center justify-center text-sm text-muted-foreground">
    <span v-if="isError">{{ t('monitoring.waits.loadInstancesError') }}</span>
    <span v-else-if="isPending || !isSuccess || (data?.instances?.length ?? 0) > 0">
      {{ t('monitoring.waits.loadingInstances') }}
    </span>
    <span v-else>{{ t('monitoring.waits.noInstances') }}</span>
  </div>
</template>
